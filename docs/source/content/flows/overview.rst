...
When you reading the official docs it's looks very intimidating, but in fact it's quite simple (by whatsbyapi 😉).
...
First you need to create the flow, give it a name and set the categories by calling :meth:`~whatsbyapi.client.WhatsApp.create_flow`:
...
.. code-block:: python
    ...
    from whatsbyapi.types import FlowCategory
...
Every component on the FlowJSON, has a corresponding class in :mod:`whatsbyapi.types.flows`:
...
After you have the flow json, you can update the flow with :meth:`~whatsbyapi.client.WhatsApp.update_flow_json`:
...
You can get the :class:`FlowDetails` of the flow with :meth:`~whatsbyapi.client.WhatsApp.get_flow` to see if there is validation errors needed to be fixed:
...
.. currentmodule:: whatsbyapi.types.callback
...
.. note::
    ...
    Once you have the public key, you can upload it using the :meth:`~whatsbyapi.client.WhatsApp.set_business_public_key` method.
    ...
    Or when installing whatsbyapi:

    >>> pip3 install "whatsbyapi[cryptography]"
...
We need to tell WhatsApp to send the requests to our serve. :meth:`~whatsbyapi.client.WhatsApp.update_flow_metadata`:
...
.. code-block:: python
    ...
    from whatsbyapi import WhatsApp
    from whatsbyapi.types import FlowButton, FlowActionType, FlowStatus
...
.. code-block:: python
    ...
    from whatsbyapi import WhatsApp
    from whatsbyapi.types import FlowRequest, FlowResponse
