...
.. code-block:: python
    ...
    from whatsbyapi.types.flows import FlowJSON
...
Creating the flow is very simple using the :meth:`~whatsbyapi.client.WhatsApp.create_flow` method:
...
WhatsApp will use to send data to our server. We can do this by using the :meth:`~whatsbyapi.client.WhatsApp.update_flow_metadata` method:
...
Finally, let's update the flow's JSON with :meth:`~whatsbyapi.client.WhatsApp.update_flow_json`:

.. code-block:: python
    :linenos:

    from whatsbyapi.errors import FlowUpdatingError
...
To send the flow we need to initialize the :class:`~whatsbyapi.client.WhatsApp` client with some specific parameters:
...
The :class:`~whatsbyapi.client.WhatsApp` class takes a few parameters:
...
.. code-block:: python
    ...
    from whatsbyapi.types import FlowButton
    from whatsbyapi.types.flows import FlowStatus, FlowActionType
...
Sending a flow is very simple. We sending text (or image, video etc.) message with a :class:`~whatsbyapi.types.callback.FlowButton`. The FlowButton contains the following properties:
...
.. code-block:: python
    ...
    from whatsbyapi.types.flows import FlowRequest, FlowResponse
...
The :meth:`~whatsbyapi.client.WhatsApp.on_flow_request` decorator takes the endpoint URI as a parameter. This is the endpoint that we provided when we updated the flow's metadata.
...
- ``wa``: The :class:`~whatsbyapi.client.WhatsApp` class instance
...
In the top of the function, we are checking if the flow request has an error. If it does, we are logging the error and returning.
    ...
    By default, if the flow has error, ``whatsbyapi`` will ignore the callback return value and will acknowledge the error.
