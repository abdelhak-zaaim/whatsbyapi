...
from whatsbyapi.handlers import Handler, ChatOpenedHandler  # noqa
from whatsbyapi.handlers import (
...
from whatsbyapi.types import MessageType, FlowRequest, FlowResponse
from whatsbyapi.types.base_update import BaseUpdate, StopHandling  # noqa
    ...
    def _get_handler(self: WhatsApp, update: dict) -> type[Handler] | None:
        ...
        if field == "messages":
            if not self.filter_updates or (
                ...
                if "messages" in value:
                    ...
                    if msg_type == MessageType.INTERACTIVE:
                        ...
                        if (
                            ...
                            return handler
                        _logger.warning(
                            "whatsbyapi Webhook: Unknown interactive message type: %s. Falling back to MessageHandler.",
                            interactive_type,
                        )
                ...
                _logger.warning("whatsbyapi Webhook: Unknown message type: %s", value)
            return None
    ...
    def _register_flow_endpoint_callback(
        ...
        if (
            ...
            raise ValueError(
                ...
                '\n>> Install it with `pip install cryptography` / pip install "whatsbyapi[cryptography]"` or use a '
