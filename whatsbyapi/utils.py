...
def default_flow_request_decryptor(
    ...
    - This implementation requires ``cryptography`` to be installed. To install it, run ``pip3 install 'whatsbyapi[cryptography]'`` or ``pip3 install cryptography``.
    ...
    Example:
        ...
        >>> from whatsbyapi.utils import default_flow_request_decryptor
...
def default_flow_response_encryptor(response: dict, aes_key: bytes, iv: bytes) -> str:
    ...
    Example:
        ...
        >>> from whatsbyapi.utils import default_flow_response_encryptor
        >>> from whatsbyapi import WhatsApp
